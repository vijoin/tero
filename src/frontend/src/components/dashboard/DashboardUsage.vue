...
<template>
  <div class="h-full flex flex-col gap-4">
    <div class="flex w-full">
      <DashboardUsageSummaryCards v-if="selectedTeam != null" :from-date="fromDate" :to-date="toDate" :team-id="selectedTeam" />
    </div>
    <div class="flex h-full w-full gap-4 min-h-0 flex-1 translate-y-1">
      <Animate :effect="AnimationEffect.SLIDE_IN_LEFT" class="h-full" :class="selectedTeam == MY_TEAM_ID ? '!w-full' : '!w-1/2'">
        <DashboardUsageCardTopAgents v-if="selectedTeam != null" :from-date="fromDate" :to-date="toDate" :team-id="selectedTeam" />
      </Animate>
      <Animate :effect="AnimationEffect.SLIDE_IN_RIGHT" class="!w-1/2" v-if="selectedTeam != null && selectedTeam != MY_TEAM_ID">
        <DashboardUsageCardTopUsers :from-date="fromDate" :to-date="toDate" :team-id="selectedTeam" />
      </Animate>
    </div>
  </div>
</template>
