...
import { AgentUsageItem, MY_TEAM_ID, Agent, UserUsageItem, PRIVATE_AGENT_ID } from '@/services/api';
...
const selectedAgent = ref<AgentUsageItem | undefined>(undefined);
...
const loadData = async () => {
    agentPage.value = 1;
    const agents = await api.getUsageTopAgents(t('privateAgents'), props.fromDate, props.toDate, props.teamId, searchAgent.value, pageSize + 1, 0);
...
const handleLoadMoreAgents = async () => {
    try {
        ...
        agentPage.value++;
        const moreAgents = await api.getUsageTopAgents(t('privateAgents'), props.fromDate, props.toDate, props.teamId, searchAgent.value, pageSize + 1, (agentPage.value - 1) * pageSize);
...
const showAgentInfo = async (agent: AgentUsageItem) => {
...
const loadTopUsers = async () => {
    ...
    try{
        userPage.value = 1;
        const users = await api.getUsageTopUsers(props.fromDate, props.toDate, props.teamId, undefined, usersPageSize + 1, 0, selectedAgent.value?.agentId);
...
const handleLoadMoreUsers = async () => {
    try{
        ...
        userPage.value++;
        const users = await api.getUsageTopUsers(props.fromDate, props.toDate, props.teamId, undefined, usersPageSize + 1, (userPage.value - 1) * usersPageSize, selectedAgent.value?.agentId);
...
<template>
    ...
    <DashboardCard v-else :title="t('agentsTitle')">
        <template #actions>
            <div class="flex items-center gap-4">
                <InteractiveInput v-model="searchAgent" :placeholder="t('searchAgent')" start-icon="IconSearch" :loading="isSearchingAgent" />
            </div>
        </template>
                ...
                <template #body="slotProps">
                    <div class="flex items-center gap-1" :class="{'border-r-1 border-auxiliar-gray pr-4': teamId != MY_TEAM_ID}">
                        <DashboardTableDataComparisonCell 
                            ...
                            :value-tooltip="t('totalChatsTooltip')" icon="IconMessages" 
                            :number-decimals="0" 
                            :compact-number="true"
                        />
                    </div>
        ...
        @load-more-data="handleLoadMoreUsers" >
            ...
            <template #summary>
                <div class="flex items-center gap-1 pr-4 border-r-1 border-auxiliar-gray">
                    <DashboardTableDataComparisonCell 
                        :previous-value="selectedAgent.previousTotalThreads" 
                        :current-value="selectedAgent.totalThreads" 
                        :value-tooltip="t('totalChatsTooltip')" icon="IconMessages" 
                        :number-decimals="0" 
                        :compact-number="true"
                    />
            ...
            <template #footerAction v-if="selectedAgent.agentId !== PRIVATE_AGENT_ID">
