...
import { Agent, RequestEvent } from "./agent"
import type { AgentRuleCondition, AddHeaderRuleAction, RecordInteractionRuleAction } from "./agent"
...
export class AgentSession {
  ...
  constructor(tabId: number, agent: Agent, url: string, id?: string, pollId?: NodeJS.Timeout) {
    this.id = id
    this.tabId = tabId
    this.agent = agent
    this.url = url
  ...
  private buildModifyHeadersRule(ruleId: number, condition: AgentRuleCondition, action: AddHeaderRuleAction): Browser.declarativeNetRequest.Rule {
    return {
      ...
      condition: {
        ...
        requestMethods: condition.requestMethods as Browser.declarativeNetRequest.RequestMethod[],
  ...
  private async findInteraction(req: RequestEvent, action: RecordInteractionRuleAction): Promise<any> {
    if (action.httpRequest) {
      ...
      const body = (req.details as Browser.webRequest.OnBeforeRequestDetails).requestBody?.raw
  ...
  public async processUserMessage(text: string, msgHandler: (part: MessagePart) => void, parentMessageId?: number) {
    const ret = this.agent.ask(text, this.id!, parentMessageId, this.authService)
    for await (const part of ret) {
      if (part.flow) {
        await new FlowExecutor(this.tabId, msgHandler).runFlow(part.flow.steps)
      } else {
        msgHandler(part)
      }
    }
    msgHandler({message: "", complete: true})
  }

  public async resumeFlow(msgHandler: (part: MessagePart) => void) {
    await new FlowExecutor(this.tabId, msgHandler).resumeFlow()
  }

  public async stopResponse(): Promise<void> {
    await this.agent.stopResponse(this.id!, this.authService)
  }

  public async transcribeAudio(audioFile: Blob): Promise<string> {
    return await this.agent.transcribeAudio(audioFile, this.id!, this.authService)
  }
