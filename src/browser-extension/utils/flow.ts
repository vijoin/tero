...
export class FlowExecutor {
  ...
  storageKey: string;
  tabId: number;
  msgHandler?: (part: MessagePart) => void

  constructor(tabId: number, msgHandler?: (part: MessagePart) => void) {
    this.tabId = tabId;
    ...
    try {
      for (let i = startIndex; i < steps.length; i++) {
        if (steps[i].action === FlowAction.MESSAGE) {
          this.msgHandler!({message: steps[i].value!});
        } else {
  ...
  public async resumeFlow(): Promise<void> {
    ...
    if (state) {
      ...
      this.msgHandler!({message: "", complete: true});
    }
  }
